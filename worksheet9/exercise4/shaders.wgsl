...
@group(0) @binding(10) var<storage> materials: array<Material>;
...
// Convert a 3D direction to equirectangular (u,v) coordinates in [0,1]
fn env_map_coords(direction: vec3f) -> vec2f {
    let Dx = direction.x;
    let Dy = direction.y;
    let Dz = direction.z;
    ...
    // light probe
    // let r = acos(-Dz) / (2 * pi * Dx * Dx + Dz * Dz);
    // let u = 0.5 + r * Dx;
    // let v = 0.5 + r * Dz; 

    // panoramic
    // let u = 0.5 + 1/(2.0 * pi) * atan2(Dx, -Dz);
    // let v = 0.5 + 1/pi * acos(-Dy);

    // scale panoramic
    let u_raw = 0.5 + atan2(Dx, -Dz) / (2.0 * pi);
    let u = u_raw - floor(u_raw);
    let v = acos(clamp(-Dy, -1.0, 1.0)) / pi;
    
    return vec2f(u, v);
}
// Sample the equirectangular environment map (flip vertical coordinate to match common images)
fn sample_environment(dir: vec3f) -> vec3f {
    var uv = env_map_coords(dir);
    let center = vec2f(0.5, 0.5);
    let scale = max(uniforms.texture_scale, 1e-6);
    var scaled = center + (uv - center) * scale;
    // flip vertical
    let final_uv = vec2f(scaled.x - 0.53, -scaled.y + 0.01);
...
fn direct_light(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    let eps = 1e-4;
    let pi = 3.141592653589793;
    let Lr = vec3f(pi, pi, pi) * 800;

    let light_origin = vec3f(-20.0, 100.0, 1.0);
    let toLight = light_origin - (*hit).position;
    let dist = length(toLight);

    var Ld = vec3f(0.0);
    if dist > 1e-6 {
        let dir = toLight / dist;
        let light = Light(Lr, dir, dist);
        let N = normalize((*hit).normal);
        let L = normalize(light.w_i);
        let cosNL = max(dot(N, L), 0.0);
        if (cosNL > 0.0) {
            // shadow test toward point light
            var shadow_tmax = max(light.dist - eps, eps);
            var shadow = Ray((*hit).position + N * eps, L, eps, shadow_tmax);
            var shHit: HitInfo;
            if (!intersect_scene(&shadow, &shHit)) {
                // point light radiance falls off with 1/r^2
                let Li = light.L_i / (light.dist * light.dist);
                let brdf = (*hit).diffuse / pi;
                Ld = brdf * Li * cosNL;
            }
        }
    }
...
fn sample_area_lights(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    ...
    let mid = face.w;
    let Le = materials[mid].emission.xyz;
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    ...
    var light = sample_area_lights(r, hit, seed);
    light += direct_light(r, hit, seed);
...
fn transparent(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    ...
    if (rnd(seed) < R) {
        ...
        // distance traveled inside medium
        let s = max((*hit).dist, 1e-4); 
        // If we are exiting the medium (hit from inside), apply Bouguer attenuation
        if (!entering && all((*hit).extinction >= vec3f(0.0))) {

            // per-channel beam transmittance Tr = exp(-sigma_t * s)
            let Tr = exp(-(*hit).extinction * s);
            let p = (Tr.x + Tr.y + Tr.z) / 3.0;

            // Russian-roulette for absorption/transmission using average Tr
            if (rnd(seed) > p) {
                (*hit).has_hit = true;
                (*hit).continue_path = false;
                return vec3f(0.0);
...
fn holdout_occlusion(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> f32 {
    let N = 16u;
    let eps = 1e-4;
    var sum = 0.0;
    let Nsurf = normalize((*hit).normal);
    for (var i = 0u; i < N; i = i + 1u) {
        ...
        let xi1 = rnd(seed);
        let xi2 = rnd(seed);
        let rr = sqrt(max(0.0, xi1));
        let phi = 2.0 * 3.141592653589793 * xi2;
        let x = rr * cos(phi);
        let y = rr * sin(phi);
        let z = sqrt(max(0.0, 1.0 - xi1));
        let localDir = vec3f(x, y, z);
        let sampleDir = rotate_to_normal(Nsurf, localDir);

        // shadow ray
        var sh = Ray((*hit).position + Nsurf * eps, sampleDir, eps, 1e9);
        var shHit: HitInfo;
        if (!intersect_scene(&sh, &shHit)) || !(intersect_min_max(&sh)) {
            sum = sum + 1.0;
        }
    }
    return sum / f32(N);
...
fn trace_rays(ipcoords: vec2f, prog_jitter: vec2f) -> Ray {
    let xip = (ipcoords.x + prog_jitter.x) * uniforms.aspect * 0.5f;
    let yip = (ipcoords.y + prog_jitter.y) * 0.5f;
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
    ...
    let safeJitters = max(1u, uniforms.no_of_jitters); // avoid 1/0
    let safeSubs = max(1.0, uniforms.subdivs); // avoid 1/0
    let rand2 = vec2f(rnd(&t), rnd(&t)) - vec2f(0.5, 0.5); // center
    let prog_jitter = rand2 * vec2f(2.0 / f32(uniforms.width), 2.0 / f32(uniforms.height)) / safeSubs;
    ...
    for (var s = 0; s < n_samples; s = s + 1) {
        let jittered_coords = coords + uniforms.jitter[s].xy;
        var r = trace_rays(jittered_coords, prog_jitter);
        ...
        for (var i = 0; i < max_depth; i = i + 1) {
            ...
            if (intersect_scene(&r, &hit)) {
                ...
                sample_result += throughput * local;
                ...
                if (t_holdout > ts.x && t_holdout < ts.y) {
                    ...
                    hit.diffuse = vec3f(0.8);
                    let occ = holdout_occlusion(&r, &hit, &t);
                    let envC = sample_environment(r.direction) * occ;
                    let sunC = direct_light(&r, &hit, &t) * 0.5;
                    sample_result += throughput * (envC + sunC);
                } else {
                    sample_result += sample_environment(r.direction);
                }
                break;
            }
        }
