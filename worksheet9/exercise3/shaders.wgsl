...
fn env_map_coords(direction: vec3f) -> vec2f {
    ...
    // azimuth (u) from -pi..pi -> 0..1, ensure wrap with fract-like behavior
    let u_raw = 0.5 + atan2(Dx, -Dz) / (2.0 * pi);
    let u = u_raw - floor(u_raw); // wrap into [0,1)
    // polar (v) from 0..pi -> 0..1
...
fn direct_light(r: ptr<function, Ray>, hit: ptr<function, HitInfo>, seed: ptr<function, u32>) -> vec3f {
    ...
    let Lr = vec3f(pi, pi, pi) * 20000;

    let light_origin = vec3f(-200.0, 100.0, 1.0);
