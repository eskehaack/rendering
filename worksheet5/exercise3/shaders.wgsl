...
fn intersect_triangle(r: Ray, hit: ptr<function, HitInfo>, v: array<vec3f, 3>, idx: vec3u) -> bool {
    let e0 = v[1] - v[0];
    let e1 = v[2] - v[0];
    let n = cross(e0, e1);
    ...
    if beta >= 0 && gamma >= 0 && (beta + gamma) <= 1 {
        // Interpolate vertex normals using triangle indices
        let w0 = 1.0 - beta - gamma;

        let n0 = meshNormals[idx.x];
        let n1 = meshNormals[idx.y];
        let n2 = meshNormals[idx.z];
...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
    // Find the closest hit in the scene (currently only triangles in the mesh).
    (*hit).has_hit = false;
    var closest = (*r).tmax;
    ...
    if intersect_plane(*r, hit, plane_position, plane_onb) {
        ...
        let texcoords = uniforms.texture_scale * vec2f(u, v);
    ...
    for (var i = 0u; i < arrayLength(&meshFaces); i = i + 1u) {
        ...
        var tempHit: HitInfo;
        // intersect_triangle takes Ray by value
        if intersect_triangle(*r, &tempHit, triangle_v, idx) {
            // Keep the nearest hit
            if (tempHit.dist < closest) {
                (*r).tmax = tempHit.dist;
                (*hit) = tempHit;
                // assign a simple color for now (can be replaced by material lookup)
                (*hit).color = vec3f(0.9);
            }
        }
    }
...
fn shade(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f { 
    switch hit.shader { 
        ...
        case 4: { return refract_shader(r, hit); }
        case 5: { return phong(r, hit); }
        case 6: { return glossy(r, hit); }
