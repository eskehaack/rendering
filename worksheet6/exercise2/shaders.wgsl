...
struct Material {
    // materials are packed as vec4 on the JS side (w pad), so use vec4f here
    emission: vec4f,
    diffuse: vec4f,
};
struct Aabb {
...
// Mesh data
@group(0) @binding(2) var<storage> vPositions: array<vec4f>; // padded vec4 per vertex
@group(0) @binding(3) var<storage> meshFaces: array<vec4u>; // v0,v1,v2,mat
@group(0) @binding(4) var<storage> meshNormals: array<vec4f>; // padded vec4 per normal
...
@group(0) @binding(7) var<storage> bspTree: array<u32>; // node*4 layout
